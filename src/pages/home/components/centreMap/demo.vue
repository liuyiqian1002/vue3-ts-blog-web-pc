...
// style
    ...
    const mapReset = (c, o, n, {data}) => {
        ...
        if (option.graphic.length == 3) {
            ...
            // let params = data.code == 100000? :{code: data.code}
            getCnAreaVehicleCount({parentCode:data.code}).then(res=>{
                ...
                option.series = [option.series[0], dataTwo, data]
    ...
    option = {
        ...
        graphic: [{
            ...
            children: [{
                ...
                transition: 'style',
                onclick: function (params) {
                    mapReset(myChart, option, opt.parentCodeName, {data: {code: areaItem.parentCode,}});
                },
...
const init = ()=>{
    return new Promise((resolve, reject) =>{
        getMap(areaItem.parentCode).then((mapJson)=>{
...
onActivated(() => {
    ...
    //         mapCode: areaItem.parentCode,       // 地图名
    //         goDown: true,       // 是否下钻
    //         // 下钻回调
    //         callback: function (name, option, instance) {
