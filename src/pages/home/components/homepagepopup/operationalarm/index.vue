<template>
    <div class="qwe">
        <el-dialog v-model="drawerFlag" title="报警趋势(营运类型)">
        ...
        <ul class="homeTitle_btns tw_cl mb10">
            ...
            <div class="provinceScreen">
              ...
              :data="gridData" size="mini"  height="435"  stripe :header-cell-style="{ background: 'rgba(6,85,105,0.3)', color: 'rgba(42, 212, 255, 1)', textAlign: 'center', fontSize: '12px', padding: '4px', }" style="width: 100%" row-style="test"
                ...
                <el-table-column prop="count" label="类型报警数"/>
                <el-table-column prop="totalCount" label="报警总次数"/>
...
import { getAlarmCountList } from '@/api/home/homepagepopup'
...
const getData = (val)=> {
    ...
    operationalarmData(val)
}
let gridData = $ref([])
let drawerFlag = $ref(false)
const operationalarmData = (val) => {
    const loading = ElLoading.service({
        lock: true,
        text: '加载中...',
        background: 'rgba(0, 0, 0, 0.7)',
    })
    getAlarmCountList({areaId: areaId, type	: val || 0}).then(res=>{
        gridData = res
        loading.close()
        drawerFlag = true
    }).catch(()=>{
...
//         operationalarmData(newValue)
//     }
// })

defineExpose({ operationalarmData })
</script>
...
.qwe {
    .homeTitle-txt {font-family: 'douyuZt';font-style:oblique;font-size: 12px;margin-top: 3px}
        .homeTitle_btns {
            ...
            //.homeTitle_btn { color: rgba(31,157,203,1);width: 44px;line-height: 21px;border: 1px solid rgba(32,153,197,1);background: rgba(31,158,204,0.2);text-align: center}
            .active { color: #fff}
        }
    :deep(.el-dialog) {
