<template>
    <div class="qwe">
        <el-dialog v-model="drawerFlag" title="7天车辆警情排行(top5)">
         <div class="homeTitle-bot relative w">
            ...
            <div class="provinceScreen">
              ...
              :data="gridData" size="mini"  height="435"  stripe :header-cell-style="{ background: 'rgba(6,85,105,0.3)', color: 'rgba(42, 212, 255, 1)', textAlign: 'center', fontSize: '12px', padding: '4px', }" style="width: 100%" row-style="test"
                ...
                <el-table-column prop="value" label="报警总次数" sortable class-name="s-c-c"/>
             </el-table>
            </div>
        </el-dialog>
    </div>
...
import { getVehicleAlarmTop5InfoList } from '@/api/home/homepagepopup'
...
const vehiclewarningData = (areaId) => {
    getVehicleAlarmTop5InfoList({areaId: areaId}).then(res=>{
        gridData = res
        drawerFlag = true
    })
}
defineExpose({ vehiclewarningData })
</script>
