        ...
        <el-dialog v-model="drawerFlag" title="风险时段">
            ...
            <div class="provinceScreen">
              ...
              :data="gridData" size="mini"  height="435"  stripe :header-cell-style="{ background: 'rgba(6,85,105,0.3)', color: 'rgba(42, 212, 255, 1)', textAlign: 'center', fontSize: '12px', padding: '4px', }" style="width: 100%" row-style="test"
                  >
                <el-table-column prop="recordTime" label="时间点" sortable class-name="s-c-c"/>
                <el-table-column prop="alarmCount" label="时段报警数" sortable class-name="s-c-c"/>
                <el-table-column prop="totalAlarmCount" label="报警总次数" sortable class-name="s-c-c"/>
                <el-table-column prop="rate" min-width label="报警占比" sortable class-name="s-c-c">
...
import { getHomePage24HourStatisticsListDto } from '@/api/home/homepagepopup'
...
const periodofriskData = (areaId) => {
    getHomePage24HourStatisticsListDto({areaId: areaId}).then(res=>{
        gridData = res
        drawerFlag = true
    })
}
